...
function ProgressListItem({ tale }: PropsWithChildren<Props>) {
  ...
  return (
    <div
      className="flex flex-col items-center gap-2 relative border-[3px] border-orange-300 bg-white rounded-lg p-3 min-w-[180px] cursor-pointer ease-in-out duration-300 hover:scale-110"
    ...
    >
      ...
      <div className="flex justify-center relative w-full mb-0.5 rounded-full bg-gray-200 dark:bg-gray-700 ">
        <div
          className="absolute top-[50%] left-[3%] translate-y-[-50%] h-[72%] rounded-full bg-green-600 dark:bg-green-500 duration-[0.33s]"
          style={{
            width: `${100 * 0.94}%`,
            transform: `width: 100%`,
          }}
        ></div>
        <div className="z-[5]">{tale.progress}%</div>
      </div>
    </div>
  )
}
